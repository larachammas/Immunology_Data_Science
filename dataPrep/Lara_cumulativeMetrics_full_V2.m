...
% files
MetricsDir_full               = dir(strcat(baseDir,'*.mat'));
numMetrics                          = size(MetricsDir_full,1);
...
for k=1: numMetrics
    ...
    caseName                = MetricsDir_full(k).name;
...
for currentCase=1: numMetrics
    ...
    % Load the current file to be displayed
    load(strcat(baseDir,MetricsDir_full(currentCase).name));
    qqq{currentCase}= MetricsDir_full(currentCase).name;
    ...
    cumulativeStats_full            = [ cumulativeStats_full; cumulativeStatsCurr];
    cumulativeTort_full             = [ cumulativeTort_full; [cumulativeStatsCurr(1,1:2) cell_metrics.pathTortuosity]]; 
end

%%
labels_full={'group','case','time','C Dist[um/s]','N Dist[um/s]', 'C numEndP', 'N numEndP_Skel', 'N numBranches_Skel', ...
...
%%
filename =strcat('cumulativeStats_full_',datestr(date,'mm_dd'));
fullfile = strcat('Results/Final_Stat_Sets_2/', filename);
save(fullfile,'cumulativeStats_full','labels_full', 'MetricsDir_full', 'cumulativeTort_full')