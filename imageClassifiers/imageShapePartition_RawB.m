...
imgFoldersB = '/Users/larachammas/OneDrive - City, University of London/IndividualProject/Data/ExtraLabeled/Labeled_Raw_b';

%% Creating Training and Testing Sets

%create an image data store
frames = imageDatastore(imgFoldersB,'FileExtensions', '.jpg','IncludeSubfolders', true, 'LabelSource', 'foldernames');

%count the number of samples in each label and find the minimum count value
countsB = countEachLabel(frames);
minLabel = min(table2array(countsB(:,2)));
...
[trainStore_RB, testStore_RB] = splitEachLabel(frames, 0.8, 'randomized');

%% make bag of features using image store data
bag_RB = bagOfFeatures(trainStore_RB); 

%make training feature vector
trainBag_RB = encode(bag_RB, trainStore_RB);

%make test feature vector
testBag_RB = encode(bag_RB, testStore_RB);

%% Create an image set 

%Create an image set 
faceSet = imageSet(imgFoldersB,'recursive');
